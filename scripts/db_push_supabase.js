...
const supabaseUrl = process.env.VITE_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://vuelgktzltmpxafkiagv.supabase.co';
const supabaseKey = process.env.VITE_SUPABASE_ANON_KEY || process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || 'sb_publishable_CKtF_wlYm9rcDtBqmreFXg_iX8Fvn7x';
...
async function dbPush() {
    ...
    try {
        ...
        if (error) {
            console.log('ℹ️ Table auto-creation notice:', error.message);
            console.log('✅ Supabase Schema SQL file generated at: supabase_schema.sql');
            console.log('👉 Please execute `supabase_schema.sql` in your Supabase SQL Editor if table auto-creation is restricted.');
        } else {
            console.log('✅ Successfully pushed seed data to Supabase!');
        }
    } catch (e) {
